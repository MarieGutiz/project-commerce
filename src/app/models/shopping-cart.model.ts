...
export default class ShoppingCarts{
    ...
    constructor(public key:string, public dateCreated:Date, private items:ShoppingCartItems[]){ 
       items?.forEach(item => {
    ...
    get totalPrice(){
      let total=0;
     this.items.forEach(item => {
      console.log("ver precio total "+item.product?.price! +" qtty" +item.quantity!)
